      ...
      <div className="w-full max-w-5xl bg-black/40 rounded-lg shadow-xl overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-2/3 p-8 backdrop-blur-sm">
          <h2 className="text-3xl font-extrabold text-white uppercase drop-shadow-lg tracking-wide">
            {isLoaded ? t("contact.title") : "Contactate con nosotros"}
          </h2>
          <p className="text-white text-base mb-6 mt-2 drop-shadow-sm">
            {isLoaded ? t("contact.description") : "Dejanos tu mensaje y nos comunicaremos con vos. También puedes "}
            {isLoaded ? (
              <>
                <a href="#equipo" className=" hover:text-white  transition-colors"> {t("contact.knowTeam")} </a>
                {t("contact.or")} <a href="/desarrolloweb" className=" hover:text-white  transition-colors">{t("contact.seeWork")}</a>.
              </>
            ) : (
              <>
          ...
          <form className="space-y-4 " onSubmit={handleSubmit} noValidate>
            {[
              { id: "nombre", label: isLoaded ? t("contact.form.name") : "Nombre", required: true },
              { id: "apellido", label: isLoaded ? t("contact.form.lastName") : "Apellido", required: true },
              { id: "empresa", label: isLoaded ? t("contact.form.organization") : "Organización", required: true },
              { id: "email", label: isLoaded ? t("contact.form.email") : "Correo Electrónico", type: "email", required: true },
            ...
            <div>
              <label htmlFor="mensaje" className="sr-only">
                {isLoaded ? t("contact.form.message") : "Mensaje"} (requerido)
              </label>
              <textarea
                id="mensaje"
                placeholder={isLoaded ? t("contact.form.message") : "Mensaje"}
                rows="4"
              ...
              <span className="relative z-10 flex items-center justify-center gap-2">
                {loading ? (
                  <>
                    ...
                    {isLoaded ? t("contact.form.sending") : "Enviando..."}
                  </>
                ) : (
                  isLoaded ? t("contact.form.sendMessage") : "Enviar Mensaje"
                )}
              </span>
            </button>
            
            <p id="submit-help" className="text-xs text-gray-300 text-center">
              {isLoaded ? t("contact.form.privacy") : "Al enviar este formulario, aceptas que procesemos tu información de contacto."}
            </p>
            ...
            {success && (
              ...
              >
                ✅ {isLoaded ? t("contact.form.success") : "¡Mensaje enviado con éxito! Te contactaremos pronto."}
              </div>
            )}
            {error && (
              <div 
        ...
        <div className="w-full lg:w-1/3 bg-purple-950 text-white p-8 flex flex-col justify-center">
          <h3 className="text-2xl font-semibold mb-4">{isLoaded ? t("contact.information") : "Información"}</h3>
          <p className="text-sm mb-2 hover:text-purple-200 hover:text-base">
            <a href="mailto:deamoncompany18@gmail.com">{isLoaded ? t("contact.generalQueries") : "Consultas generales"}</a>
         
          </p>
          <p className="text-sm mb-2 hover:text-purple-200 hover:text-base">
          <a href="mailto:serv.levelup@gmail.com">{isLoaded ? t("contact.marketingQueries") : "Consultas de Marketing Digital"}</a>
          </p>
          <p className="text-sm mb-4 hover:text-purple-200 hover:text-base">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer">
              {isLoaded ? t("contact.writeWhatsApp") : "Escribinos un WhatsApp"}
            </a>
          </p>
          ...
          <p className="text-sm mb-4 hover:text-purple-200 hover:text-base">
            ...
            >
              {isLoaded ? t("contact.scheduleMeeting") : "Agenda una reunión"}
            </a>
          </p>

          <h4 className="text-lg font-medium mt-4">Deamon DD</h4>
          <p className="text-sm">{isLoaded ? t("contact.company.tagline") : "Composición & Construcción"}</p>
          <p className="text-xs mt-2">
            {isLoaded ? t("contact.company.description") : "Especialistas en "}
            {isLoaded ? (
              <>
                <a href="/desarrolloweb" className=" hover:text-white  transition-colors">{t("contact.company.systems")}</a>
                {t("contact.company.and")} <a href="/graphic" className=" hover:text-white  transition-colors">{t("contact.company.visual")}</a> {t("contact.company.location")}.
              </>
            ) : (
              <>
                <a href="/desarrolloweb" className=" hover:text-white  transition-colors">construcción de sistemas </a>
          ...
          <div className="mt-3 text-left">
            ...
            <p className="text-xs ml-1 font-light">{isLoaded ? t("contact.company.founder") : "Fundador Deamon DD"}</p>
          </div>
        </div>
      </div>
    </div>
