    ...
    >
      <div className="w-full max-w-5xl bg-black/40 rounded-lg shadow-xl overflow-hidden flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/3 bg-purple-950 text-white p-8 flex flex-col justify-center">
          ...
          {/* Botón Agenda una reunión */}
          <button
            onClick={() => {
              const reunionMessage = "Hola! Me gustaría agendar una reunión para conocer más sobre los servicios de Deamon DD. 💼";
              const reunionWhatsappLink = `https://wa.me/${phone}?text=${encodeURIComponent(reunionMessage)}`;
              window.open(reunionWhatsappLink, '_blank');
            }}
            className="bg-gradient-to-r from-violet-800 to-violet-600 hover:from-violet-600 hover:to-violet-700 text-white px-6 py-3 rounded-lg font-semibold flex items-center gap-2 transition-all duration-300 shadow-lg shadow-pink-500/20 mb-4"
          >
            Agenda una reunión
