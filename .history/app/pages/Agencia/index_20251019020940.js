...
function AgenciaContent() {
  ...
  if (!isLoaded) {
    return <div className="w-full min-h-screen bg-gradient-to-br from-black via-black to-black text-white flex items-center justify-center">
      <div className="text-white">Cargando...</div>
    </div>;
  }
  
  // Obtener las traducciones directamente
  const title = t("agency.title");
  const titleHighlight = t("agency.titleHighlight");
  const description = t("agency.description");
  const webDev = t("agency.services.webDev");
  const branding = t("agency.services.branding");
  const marketing = t("agency.services.marketing");
  const services = t("agency.code.services");
  const mission = t("agency.code.mission");
  const passion = t("agency.code.passion");
  const improveClient = t("agency.code.improveClient");
  const creativityResults = t("agency.code.creativityResults");
  
  console.log('🔍 AGENCIA - Traducciones:', { title, description, webDev, branding, marketing });
...
// Componente principal que maneja el re-render
export default function TeamSection() {
  const { currentLang } = useTranslation();
  const [renderKey, setRenderKey] = useState(0);
  
  // Debug temporal
  console.log('🔍 TEAM SECTION - currentLang:', currentLang, 'renderKey:', renderKey);
  
  // Forzar re-render cuando cambia el idioma
  useEffect(() => {
    console.log('🔄 TEAM SECTION - useEffect triggered, currentLang:', currentLang);
    setRenderKey(prev => prev + 1);
  }, [currentLang]);
  
  // Renderizar directamente el contenido con key dinámica
  return (
    <div key={`${currentLang}-${renderKey}`}>
      <AgenciaContent />
    </div>
  );
}
