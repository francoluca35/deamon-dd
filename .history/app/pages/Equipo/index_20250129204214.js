...
function HoverImage({ defaultImage, hoverImage, alt }) {
  ...
  return (
    <Image
      ...
      width={860}
      height={60}
      className="w-full h-40 object-cover rounded-lg mb-4 transition-all duration-300"
    ...
    <section className="bg-black text-white py-12 px-6">
      ...
      <div className="flex justify-center gap-8">
        {teamMembers.map((member, index) => (
          <div key={index} className="bg-gray-900 p-6 rounded-lg w-80 h-80 text-center relative">
            <HoverImage className="h-80 w-80" defaultImage={member.image} hoverImage={member.hoverImage} alt={member.name} />
