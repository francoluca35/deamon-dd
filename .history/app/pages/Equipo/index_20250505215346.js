...
export default function TeamSection() {
  ...
  return (
    <div className="w-full min-h-screen bg-gray-900/30 flex flex-col items-center justify-center px-4 md:px-8">
      <div className="flex flex-col md:flex-row items-center justify-center w-full max-w-7xl gap-10 text-center md:text-left mt-20">
      ...
      <div
        ref={teamRef}
        className="w-full max-w-7xl grid grid-cols-1 sm:grid-cols-2 gap-10 mt-12 px-4"
      >
        {teamMembers.map((member, index) => (
          <motion.div
            key={index}
            className="relative group w-full rounded-xl mx-auto min-h-[400px] overflow-hidden"
          >
            <img
              src={member.img}
              alt={member.name}
            ...
            />
            <div className="absolute inset-0 bg-purple-900/20 flex flex-col items-center justify-center transition opacity-100 group-hover:opacity-0">
              <p className="text-black border border-white px-4 py-2 rounded-2xl bg-white/60 text-lg font-light">
                {member.name}
              </p>
            </div>
            <motion.div
            ...
            >
              <p
                className="text-black text-sm md:text-lg text-center whitespace-pre-line max-w-xs md:max-w-full"
