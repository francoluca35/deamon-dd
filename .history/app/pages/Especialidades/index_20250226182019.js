...
export default function Especialidades() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const elements = document.querySelectorAll('.especialidad');
      ...
      <section className="bg-black text-white py-16 px-4 flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-20 ">
          {especialidades.map((item, index) => (
            <div
              key={index}
              className={`especialidad bg-purple-900/40 opacity-0 rounded-xl p-6 text-center flex flex-col items-center relative overflow-visible w-full sm:w-80 md:w-auto h-72 md:h-80 transition-opacity duration-500 ease-in-out ${
                visible[index] ? 'opacity-100' : ''
              }`}
              style={{ transitionDelay: `${index * 100}ms` }} // Añade un pequeño retraso para cada uno
            >
