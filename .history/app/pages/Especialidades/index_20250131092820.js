      ...
      <section className="bg-black text-white py-12 px-6 flex justify-center">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
          {especialidades.map((item, index) => (
            <div key={index} className="bg-gray-800 rounded-xl p-4 text-center flex flex-col items-center">
              <h3 className="text-xl font-bold mb-4">{item.titulo}</h3>
              <img src={item.imagen} alt={item.titulo} className="w-full h-auto rounded-lg" />
            </div>
          ))}
        </div>
      </section>
