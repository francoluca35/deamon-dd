    ...
    return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl">
          {especialidades.map((item, index) => (
            <div key={index} className="bg-gray-800 rounded-xl p-8 text-center flex flex-col items-center relative overflow-visible w-screen h-80">
              <h3 className="text-xl font-bold mb-4 relative z-10">{item.titulo}</h3>
              <p>loremdgsjachdgljglk</p>
              <img src={item.imagen} alt={item.titulo} className="w-1w-4/5 h-auto rounded-lg absolute bottom-[-30px] shadow-lg z-20" />
            </div>
          ))}
        </div>
      </section>
