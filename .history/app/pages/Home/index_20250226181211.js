...
export default function Home() {
  // Estado para la opacidad de la imagen
  const [scrollY, setScrollY] = useState(0);

  useEffect(() => {
    // Función para actualizar el valor de scrollY
    const handleScroll = () => {
      setScrollY(window.scrollY); // Guardamos el valor del scroll
    };
  ...
  // Calculamos la opacidad de la imagen en función del desplazamiento
  const opacity = Math.max(1 - scrollY / 500, 0); // Hace que la imagen desaparezca con el scroll
      ...
      <div className="ml-10 md:ml-44 z-10 text-left text-white w-auto">
        ...
        <div className="block md:hidden w-full flex justify-center mb-4">
          <Image
            ...
            style={{ opacity }} // Aplicamos la opacidad dinámica
          />
        </div>
