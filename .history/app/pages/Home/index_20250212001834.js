import Image from 'next/image';

export default function Home() {
  return (
    <div className="relative w-full h-screen flex items-center justify-center bg-cover bg-center p-6 md:p-0" style={{ backgroundImage: 'url(/assets/fondo.png)' }}>
      <div className="absolute inset-0 bg-black bg-opacity-40"></div>

      {/* Contenedor de texto */}
      <div className="relative z-10 text-center md:text-left text-white max-w-2xl">
        <h1 className="text-4xl md:text-6xl font-bold mb-4 leading-tight">
          HACEMOS DE TU MARCA ALGO ÚNICO
        </h1>
        <p className="text-lg md:text-xl mb-6">
          Somos una agencia de diseño & desarrollo web, nuestro único propósito es ayudarte a potenciar tu empresa.
        </p>

        {/* Imagen única en responsive (mobile/tablet) */}
        <div className="block md:hidden w-full flex justify-center mb-6">
          <Image
            src="/assets/pages.png"
            width={400}
            height={300}
            alt="Mockup Responsive"
            className="object-contain rounded-lg"
          />
        </div>

        <button className="px-6 py-3 border border-purple-500 text-purple-500 hover:bg-purple-500 hover:text-white transition rounded-lg">
          NUESTRO EQUIPO
        </button>
      </div>

      {/* Imágenes flotantes solo en desktop */}
      <div className="hidden md:flex relative w-full justify-center items-center mt-8 md:mt-16">
        <div className="absolute transform rotate-[-10deg] z-10 left-[5%] lg:w-[40%] scale-105 hover:scale-110 transition">
          <Image
            src="/assets/pages.png"
            width={900}
            height={600}
            alt="Mockup 1"
            className="object-contain"
          />
        </div>
        <div className="absolute transform rotate-[-15deg] z-10 left-[50%] top-[-10%] w-[35%] scale-105 hover:scale-110 transition">
          <Image
            src="/assets/joa.jpg"
            width={600}
            height={600}
            alt="Mockup 2"
            className="object-contain rounded-[10%]"
          />
        </div>
        <div className="absolute transform rotate-[10deg] scale-x-[-1] z-10 left-[40%] top-[40%] w-[30%] scale-105 hover:scale-110 transition">
          <Image
            src="/assets/dessing.png"
            width={500}
            height={600}
            alt="Mockup 3"
            className="object-contain"
          />
        </div>
      </div>
    </div>
  );
}
