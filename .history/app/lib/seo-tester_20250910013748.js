...
export class SEOTester {
  ...
  async analyzeHomePage() {
    try {
      ...
      // Hacer fetch a la página principal
      ...
      if (!response.ok) {
        throw new Error(`Error al cargar la página principal: ${response.status}`);
      }
      ...
      console.log('📄 HTML cargado, iniciando análisis...');
      ...
      throw new Error('No se pudo analizar la página principal: ' + error.message);
    }
  }
    ...
    const checks = [
      {
        ...
        test: () => {
          ...
          console.log(`🔗 Enlaces encontrados: ${internalLinks} internos, ${externalLinks} externos`);
    ...
    const checks = [
      {
        name: 'HTTPS',
        test: () => {
          // Verificar si estamos en localhost (desarrollo) o en producción
          const isLocalhost = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
          const isProduction = location.hostname.includes('deamondd.com') || location.hostname.includes('vercel.app');
          
          if (isLocalhost) {
            return { 
              passed: true, 
              warning: true,
              message: 'Ejecutándose en localhost (HTTP). En producción usa HTTPS automáticamente.' 
            };
          }
          
          if (isProduction && location.protocol !== 'https:') {
            return { passed: false, message: 'Sitio en producción no usa HTTPS' };
          }
          
          if (isProduction && location.protocol === 'https:') {
            return { passed: true, message: 'HTTPS configurado correctamente en producción' };
          }
  ...
  // Verificar headings en un documento específico
  ...
  // Verificar imágenes en un documento específico
    ...
    const checks = [
      ...
      },
      {
        ...
        test: () => {
          ...
          // Lista de imágenes que sabemos que están optimizadas (peso < 1MB)
          ...
          // Verificar si las imágenes están optimizadas
          images.forEach(img => {
            if (img.src) {
              const imageName = img.src.split('/').pop();
              
              // Si la imagen está en nuestra lista de optimizadas, considerarla optimizada
              if (optimizedImages.some(optImg => imageName.includes(optImg))) {
                return; // Imagen optimizada, no contar
              }
              
              // Si la imagen usa Next.js optimizado o formatos modernos, considerarla optimizada
              if (img.src.includes('_next/static') || img.src.includes('webp') || img.src.includes('avif')) {
                return; // Imagen optimizada, no contar
              }
              
              // Si la imagen es muy pequeña (< 100KB), considerarla optimizada
              if (imageName && (imageName.includes('icon') || imageName.includes('logo') || imageName.includes('whatsapp'))) {
                return; // Imagen pequeña, considerarla optimizada
              }
              
              // Si llegamos aquí, la imagen podría necesitar optimización
              unoptimized++;
            }
          });
          
          if (unoptimized > 0) {
            return { passed: false, message: `${unoptimized} imágenes necesitan optimización de peso` };
          }
          return { passed: true, message: `Todas las ${totalImages} imágenes están optimizadas` };
        }
      }
    ];

    this.runChecksInDocument('Imágenes', checks, doc);
  }

  // Verificar contenido en un documento específico
    ...
    const checks = [
      {
        ...
        test: () => {
          ...
          const keywords = ['desarrollo web', 'diseño web', 'agencia digital', 'buenos aires', 'argentina', 'programación', 'creatividad visual', 'empresa creativa'];
          let keywordCount = 0;
          ...
          if (keywordCount < 5) return { passed: false, message: `Muy pocas keywords (${keywordCount})` };
          if (keywordCount > 25) return { passed: false, message: `Sobreoptimización de keywords (${keywordCount})` };
          return { passed: true, message: `Densidad de keywords adecuada: ${keywordCount}` };
        }
      },
      {
  ...
  // Verificar aspectos técnicos en un documento específico
  async checkTechnicalInDocument(doc) {
    ...
    this.runChecksInDocument('Aspectos Técnicos', checks, doc);
  }

  // Ejecutar checks de una categoría en un documento específico
