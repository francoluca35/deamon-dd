    ...
    const checks = [
      ...
      },
      {
        ...
        test: () => {
          ...
          // Verificar si las imágenes usan Next.js Image component
          images.forEach(img => {
            // Si la imagen tiene src que no es de Next.js optimizado
            if (img.src && !img.src.includes('_next/static') && !img.src.includes('webp') && !img.src.includes('avif')) {
              // Verificar si es una imagen muy pesada (más de 200KB)
              // Como no podemos verificar el tamaño real, asumimos que si no está optimizada por Next.js, necesita optimización
              unoptimized++;
            }
          });
