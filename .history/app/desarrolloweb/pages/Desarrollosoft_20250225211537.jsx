...
function Button({ children, onClick, isActive }) {
  return (
    <button
      className={`px-5 py-2 rounded-md transition-all ${isActive ? 'bg-black text-white' : 'bg-gray-200 text-black hover:bg-gray-300'}`}
      onClick={onClick}
    >
      {children}
    </button>
...
function Card({ title, category, image }) {
  return (
    <div className="relative group overflow-hidden  shadow-lg ">
      <img src={image} alt={title} className="  object-cover transition-transform duration-300 group-hover:scale-110" />
