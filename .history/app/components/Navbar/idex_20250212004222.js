...
function Navbar2() {
  ...
  return (
    <nav className={`${isScrolled ? "bg-black shadow-md" : "bg-transparent"} fixed w-full z-20 top-0 border-b border-gray-200`}>
              ...
              <li key={item.name}>
                <Link 
                  ...
                  className={`block py-2 px-3 rounded-sm transition ${
                    router.pathname === item.path ? "text-[#ba73f5]" : "text-gray-900 hover:text-[#ba73f5]"
                  }`}
                >
                  {item.name}
                </Link>
