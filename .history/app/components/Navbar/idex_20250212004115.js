  ...
  return (
    <nav className={`${isScrolled ? "bg-black shadow-md" : "bg-transparent"} fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600`}>
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        {/* Menú Hamburguesa */}
        <button onClick={() => setIsOpen(!isOpen)} className="inline-flex md:hidden p-2 text-white rounded-lg hover:bg-gray-100">
              ...
              <li key={item}>
                <a 
                  ...
                  className={`block py-2 px-3 rounded-sm transition ${
                    active === item ? "text-[#ba73f5]" : "text-white hover:text-[#ba73f5]"
                  }`}
                >
                  {item}
                </a>
