...
const Navbar2 = memo(function Navbar2() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Solo mostrar navbar cuando haces scroll hacia arriba
      if (currentScrollY < lastScrollY && currentScrollY > 50) {
        // Scroll hacia arriba y más de 50px desde arriba = mostrar navbar
        setIsNavbarVisible(true);
      } else if (currentScrollY > lastScrollY) {
        // Scroll hacia abajo = ocultar navbar
        setIsNavbarVisible(false);
      } else if (currentScrollY < 50) {
        // Si estamos muy arriba (menos de 50px), ocultar navbar
        setIsNavbarVisible(false);
      }
    ...
    <nav
      className={`${
        isMobile
          ...
          ? "bg-black bg-opacity-40"
          : "bg-transparent"
      } fixed w-full z-50 top-0 start-0 transition-transform duration-300 ease-in-out ${
    ...
    >
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <button 
          ...
          className="flex items-center focus:outline-none hover:scale-105 transition-all duration-300 focus:ring-offset-2 rounded-lg p-1"
        ...
        <div className="flex lg:order-2 space-x-3 lg:space-x-0 rtl:space-x-reverse">
          {/* Selector de idiomas - Solo visible en desktop */}
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 ${
                  currentLang === lang.code
                    ? 'ring-0 ring-purple-500 ring-offset-2 ring-offset-transparent shadow-lg shadow-purple-500/30'
