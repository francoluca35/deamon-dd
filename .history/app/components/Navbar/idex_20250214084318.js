...
function Navbar2() {
  ...
  const navLinks = [
    ...
    { href: "/dev", label: "Desarrollo Web", isRoute: true },  // Asegurar que es una ruta válida
  ...
  const handleLinkClick = (href, isRoute) => {
    setActiveLink(href);
    if (isRoute) {
      router.push(href); // Redirige a una nueva página
    } else {
      window.location.href = href; // Para secciones dentro de la misma página
    }
  };
