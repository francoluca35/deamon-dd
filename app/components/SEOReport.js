    ...
    <div className="max-w-6xl mx-auto p-6 bg-white">
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                console.log('Title en página principal:', doc.querySelector('title')?.textContent);
                console.log('Enlaces en página principal:', doc.querySelectorAll('a[href]').length);
                
                // Mostrar enlaces encontrados
                const links = doc.querySelectorAll('a[href]');
                links.forEach((link, index) => {
                  console.log(`Enlace ${index + 1}:`, link.getAttribute('href'), link.textContent?.trim());
                });
      ...
      {!report && !loading && !error && (
        <div className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded mb-6">
          <div className="flex items-center gap-2">
            <span>ℹ️</span>
            <span>Haz clic en &quot;Ejecutar Test SEO&quot; para comenzar el análisis de tu sitio web.</span>
          </div>
          <div className="mt-2 text-sm text-blue-700">
            <strong>Nota:</strong> El test analizará automáticamente la página principal (/) para obtener resultados precisos.
          </div>
        </div>
      )}
      ...
      {loading && (
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded mb-6">
          <div className="flex items-center gap-2">
            ...
            <span>Analizando la página principal de tu sitio web...</span>
          </div>
        </div>
      )}
